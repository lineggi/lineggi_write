...
""".strip()
    ...
    # 강의 홍보 CTA (후크→스토리→제안)
    # -----------------------------
    def build_cta(self, short_title: str) -> str:
        """글 끝에 붙일 강의 홍보 문단을 생성한다(URL 제외, 뒤에서 붙임).

        마케팅 퍼널 원칙(후크→스토리→제안, 문제 자극, 행동 유도)을 따른다.
        생성 실패 시 정적 문구로 대체한다.
        """
        fallback = (
            f"{short_title}, 뉴스로 흐름을 아는 것과 실제로 수익을 내는 건 다르다. "
            "대부분은 무슨 일이 있었는지까지만 알고 멈춘다. "
            "크립토유치원 강의는 초보도 따라 할 수 있는 투자 원칙과 실전 전략을 단계별로 정리해준다. "
            "지금 바로 확인해보라."
        )
        prompt = f"""
너는 세일즈 카피라이터다. 방금 발행한 크립토 뉴스 글 끝에 붙일, 내 강의를 홍보하는 짧은 마무리 문단을 쓴다.

[방금 글의 주제] {short_title}

[구성: 후크 → 스토리 → 제안]
- 후크: 이 글을 읽은 독자의 욕구나 불안을 한 문장으로 건드린다(뉴스를 아는 것과 실제 수익은 다르다는 취지).
- 스토리/문제: 대부분은 '무슨 일이 있었나'까지만 알고 멈춘다는 점을 짚는다.
- 제안: '크립토유치원' 강의가 초보도 따라 할 수 있는 투자 원칙과 실전 전략을 준다는 점을 제시한다.
- 마지막 문장은 지금 확인해보라는 부드러운 행동 유도로 끝낸다.

[문체 규칙]
- 평어체(~다). 3~4문장. 과장·클릭베이트 단어 금지. em dash(—) 금지.
- URL·링크는 절대 쓰지 마라(뒤에 따로 붙는다).
- '후크/스토리/제안' 같은 라벨을 출력하지 말고 자연스러운 한 문단으로만 써라.
""".strip()
        out = self._safe_generate(prompt)
        if not out or out.startswith("❌"):
            return fallback
        return _strip_em_dash(out.strip())
